...
from PyQt6.QtWidgets import (QDockWidget, QWidget, QVBoxLayout, QPushButton, 
                           QLabel, QComboBox, QLineEdit, QHBoxLayout, QListWidget,
                           QListWidgetItem)
from PyQt6.QtCore import Qt, QSize, QMimeData, QPoint
from PyQt6.QtGui import QIcon, QDrag, QPixmap
from utils.styles import load_dark_theme, load_light_theme
...
# class DraggableButton(QPushButton):       20jan-1
...
class DraggableButton(QPushButton):
    def mousePressEvent(self, event):
        if event.button() == Qt.MouseButton.LeftButton:
            drag = QDrag(self)
            mime_data = QMimeData()
            mime_data.setText(self.toolTip())
            drag.setMimeData(mime_data)
            ...
            if drag.exec(Qt.DropAction.CopyAction | Qt.DropAction.MoveAction) == Qt.DropAction.MoveAction:
                print("Moved")
            else:
                print("Copied")
        else:
    ...
    def mouseReleaseEvent(self, event):
        ...
        if self.parent() and hasattr(self.parent(), 'toggle_sidebar'):
            self.parent().parent().toggle_sidebar()
...
# components/sidebar.py
# [Previous code stays the same until DesignSidebar class]
